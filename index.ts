    ...
    api.registerCommand({
      ...
      async handler(_ctx: PluginCommandContext) {
        try {
          ...
          if (epochs.length === 0) return reply(`✅ **All epochs settled.** No pending chain submissions.`);
          ...
          const { Connection, Keypair, PublicKey, Transaction, TransactionInstruction, SystemProgram, SYSVAR_RENT_PUBKEY, sendAndConfirmTransaction } = await import('@solana/web3.js');
          const { getAssociatedTokenAddress, createAssociatedTokenAccountInstruction, getAccount, TOKEN_PROGRAM_ID } = await import('@solana/spl-token');
          const crypto = await import('crypto');

          const solanaRpcUrl = (pluginConfig as any).solanaRpcUrl ?? 'https://api.devnet.solana.com';
          const rewardProgramId = (pluginConfig as any).rewardProgramId ?? '2sw21aMVVodkuZinMpqk9EZSM5NFTpbtvFqb1K89wPy8';
          const rewardMint = (pluginConfig as any).rewardMint ?? '4vfddkdq6ZFnFKaKeHTSmja1P34GYoBvipycq5wmhqst';
          ...
          for (const epoch of epochs) {
            try {
              ...
              // 确保 vault ATA 存在
              try { await getAccount(connection, vault); }
              catch {
                const createVaultTx = new Transaction().add(
                  createAssociatedTokenAccountInstruction(keypair.publicKey, vault, epochConfigPDA, mint)
                );
              ...
              let off = 0;
              argsBuf.writeBigUInt64LE(BigInt(epoch.epoch_id), off); off += 8;
              rootBytes.copy(argsBuf, off); off += 32;
              argsBuf.writeBigUInt64LE(BigInt(epoch.reward_pool), off); off += 8;
              argsBuf.writeBigInt64LE(BigInt(now), off); off += 8;
              ...
              const sig = await sendAndConfirmTransaction(
                connection, new Transaction().add(ix), [keypair], { commitment: 'confirmed' }
              );
              ...
              lines.push(`❌ Epoch #${epoch.epoch_id} failed: ${(epochErr as Error).message.slice(0, 80)}`);
            }
          }
